...
public:
	...
	void SetSSL(bool b,int ty)
	{
		S = b;
		Type = ty;
	}

	void SetExtCert(PCCERT_CONTEXT pc = 0,HCERTSTORE str = 0)
	{
		OurCertificate = pc;
		OurStore = str;
		IsExternalCert = pc != nullptr;	
	}
	...
	int transmit(const char* b, int sz, bool ForceAll = false, int p = 0, std::function<void(int)> cb = nullptr,bool ForceRaw = 0)
	{
		if (S && Agreed && !ForceRaw)
...
//				tosend = 10000;
	...
	int receive(char* b, int sz, bool ForceAll = false, int p = 0,bool ForceRaw = 0)
	{
		if (S && !ForceRaw && Agreed)
	...
	{
		return transmit(b, sz, ForceAll, p, cb,1);
	}
	...
	{
		return receive(b, sz, ForceAll, p,true);
	}

	int stransmit(const char* b, int sz, std::vector<char>* rm = 0)
	{
		if (!S)
			return transmit(b, sz, true);
	...
	int Client()
	{
		...
		if (hCred.dwLower == 0 && hCred.dwUpper == 0)
		{
			...
			if (OurCertificate)
			{
				...
				m_SchannelCred.hRootStore = OurStore;
			}

			ss = AcquireCredentialsHandleW(0, (LPWSTR)SCHANNEL_NAME, SECPKG_CRED_OUTBOUND, 0, &m_SchannelCred, 0, 0, &hCred, 0);
			if (FAILED(ss))
				return 0;
		}
	...
	PCCERT_CONTEXT CreateOurCertificate()
	{
		...
		try
		{
			...
			sib.pbData = (BYTE*)cb;
			sib.cbData = 1000;
			const wchar_t* szSubject = L"CN=TurboPlay";
...
/*		SCH_CREDENTIALS creds = {};
		...
		creds.cCreds = 1;
		creds.paCred = &OurCertificate;
		creds.dwFlags = 0;
		*/
		ss = AcquireCredentialsHandleW(0, (LPWSTR)UNISP_NAME, SECPKG_CRED_INBOUND, 0, &m_SchannelCred, 0, 0, &hCred, 0);
		if (FAILED(ss))
			return -1;
		...
		for (;;)
		{
			...
			// Pass data to the remote site
			unsigned int rval2 = (unsigned int)transmit_raw((char*)bufso[0].pvBuffer, bufso[0].cbBuffer, true);
			FreeContextBuffer(bufso[0].pvBuffer);
